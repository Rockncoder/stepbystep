...
module webServer {
  ...
  webServer.$inject = ['$httpBackend'];

  function webServer($httpBackend: ng.IHttpBackendService) {
    console.info('in webServer');

    // asking for a single quiz - method, url
    $httpBackend.whenGET(/\/api\/quiz\/\d+/).respond((method, url) => {
      let params = regex.exec(url);
      let id = parseInt(params[1], 10);

      // the regex guarantees that there is an id number, we need to make sure it is in range
      if ( id > 0 && id <= quizList.length) {
        // return the data for the quiz
        return [200, quizList[id - 1]];
      }
      //
      return [404, {}, {}, `Quiz ${params[1]} not found`];
    });

    // asking for the quiz listings
    $httpBackend.whenGET('/api/quiz').respond(() => {
      console.info("in '/api/quiz'");
      // extract only the relevant data
      let strippedQuizzes = quizList.map(el =>({
          title: el.title,
          tagLine: el.tagLine,
          added: el.added,
          tags: el.tags,
      ...
      return [200, strippedQuizzes];
    });

    // for everything else, just pass it through
    $httpBackend.whenGET('').passThrough();
  }

  console.info('fake-webserver.ts');
}